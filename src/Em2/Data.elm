module Em2.Data exposing (SkillGroup, cv, skillGroups)

{-| Experimental EM variant at `/em2`. Same designed two-page layout as `/em`
(renders through `Cv.View`), but each role leads with explicit `Scope:` and
`Stack:` lines (like the ATS cut), and the bullets are trimmed of anything that
now lives in those lines, to offset the added length.

Scope absorbs team size / area / roadmap-ownership; Stack absorbs the
technology list; the bullets keep only the achievements and detail not already
captured above. Same honesty guardrails as `/em` (see Em.Data).
...
{-| Grouped Core Capabilities, lifted from the ATS cut (`/em-ats`): labelled
groups rendered as comma-separated lines rather than the two-column phrase
list. "Languages & Platform" carries the technical stack, so the standalone
Technical Skills line is dropped on this variant.
-}
...
skillGroups =
    [ { name = "Leadership"
      , skills =
            ...
            , "Agile (Shape Up / Scrum / Kanban / BDD)"
            ]
      }
